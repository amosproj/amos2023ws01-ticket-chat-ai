...
class CategoryRepository:
    ...
    def update_category(
        self, category_id: ObjectId, category: CategoryEntity | dict
    ) -> UpdateResult:
        logger.info(f"Updating category {category_id} in the database...")
        return self.collection.replace_one(
            filter={"_id": category_id}, replacement=category, upsert=True
        )
