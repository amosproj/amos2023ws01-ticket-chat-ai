...
class UserDBRoutineServiceUnitTest(TestCase):
    def setUp(self):
        ...
        self.user_db_service = UserDBRoutineService(user_repository=self.user_repository_mock)

    def test_start_routine(self):
        # Act
        self.user_db_service.start_routine()
        # Assert
        self.assertEqual(10, len(self.user_repository_mock.create_user.mock_calls), "not all defaults saved")
